...
class CGAN(object):
    ...
    def build_model(self):
        ...
        with tf.name_scope('d_loss'):
            ...
            pos_loss = tf.reduce_mean(
                tf.square(pos - tf.random.uniform(shape=[self.batch_size, 1], minval=0.7, maxval=1.2)))
            ...
            neg_loss = tf.reduce_mean(
                tf.square(neg - tf.random.uniform(shape=[self.batch_size, 1], minval=0, maxval=0.3, dtype=tf.float32)))
        ...
        with tf.name_scope('g_loss'):
            ...
            self.g_loss_1 = tf.reduce_mean(tf.square(
                neg - tf.random.uniform(shape=[self.batch_size, 1], minval=0.7, maxval=1.2, dtype=tf.float32)))
            ...
            self.g_loss_2 = tf.reduce_mean(tf.square(self.fusion_image - self.labels_ir)) + 5 * tf.reduce_mean(
                tf.square(gradient(self.fusion_image) - gradient(self.labels_vi)))
    ...
    def train(self, config):
        ...
        config.checkpoint_dir = "checkpoint_20"
        if config.is_train:
        ...
        with tf.name_scope('train_step'):
            ...
            self.train_discriminator_op = tf.compat.v1.train.AdamOptimizer(config.learning_rate).minimize(self.d_loss,
                                                                                                var_list=self.d_vars)
        # 将所有统计的量合起来
        self.summary_op = tf.compat.v1.summary.merge_all()
        # 生成日志文件
        self.train_writer = tf.compat.v1.summary.FileWriter(config.summary_dir + '/train', self.sess.graph, flush_secs=60)
        ...
        if config.is_train:
            ...
            for ep in range(config.epoch):
                ...
                for idx in range(0, batch_idxs):
                    ...
                    counter += 1
                    for i in range(2):
                        _, err_d = self.sess.run([self.train_discriminator_op, self.d_loss],
    ...
    def fusion_model(self, img):
        with tf.compat.v1.variable_scope('fusion_model'):
            with tf.compat.v1.variable_scope('layer1'):
                weights = tf.compat.v1.get_variable("w1", [5, 5, 2, 256],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights)
                bias = tf.compat.v1.get_variable("b1", [256], initializer=tf.compat.v1.constant_initializer(0.0))
                conv1_ir = tf.nn.conv2d(img, weights, strides=[1, 1, 1, 1], padding='VALID') + bias
                conv1_ir = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv1_ir)
                conv1_ir = lrelu(conv1_ir)
            with tf.compat.v1.variable_scope('layer2'):
                weights = tf.compat.v1.get_variable("w2", [5, 5, 256, 128],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights)
                bias = tf.compat.v1.get_variable("b2", [128], initializer=tf.compat.v1.constant_initializer(0.0))
                conv2_ir = tf.nn.conv2d(conv1_ir, weights, strides=[1, 1, 1, 1], padding='VALID') + bias
                conv2_ir = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv2_ir)
                conv2_ir = lrelu(conv2_ir)
            with tf.compat.v1.variable_scope('layer3'):
                weights = tf.compat.v1.get_variable("w3", [3, 3, 128, 64],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights)
                bias = tf.compat.v1.get_variable("b3", [64], initializer=tf.compat.v1.constant_initializer(0.0))
                conv3_ir = tf.nn.conv2d(conv2_ir, weights, strides=[1, 1, 1, 1], padding='VALID') + bias
                conv3_ir = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv3_ir)
                conv3_ir = lrelu(conv3_ir)
            with tf.compat.v1.variable_scope('layer4'):
                weights = tf.compat.v1.get_variable("w4", [3, 3, 64, 32],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights)
                bias = tf.compat.v1.get_variable("b4", [32], initializer=tf.compat.v1.constant_initializer(0.0))
                conv4_ir = tf.nn.conv2d(conv3_ir, weights, strides=[1, 1, 1, 1], padding='VALID') + bias
                conv4_ir = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv4_ir)
                conv4_ir = lrelu(conv4_ir)
            with tf.compat.v1.variable_scope('layer5'):
                weights = tf.compat.v1.get_variable("w5", [1, 1, 32, 1],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights)
                bias = tf.compat.v1.get_variable("b5", [1], initializer=tf.compat.v1.constant_initializer(0.0))
    ...
    def discriminator(self, img, reuse, update_collection=None):
        with tf.compat.v1.variable_scope('discriminator', reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('layer_1'):
                weights = tf.compat.v1.get_variable("w_1", [3, 3, 1, 32],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights, update_collection=update_collection)
                bias = tf.compat.v1.get_variable("b_1", [32], initializer=tf.compat.v1.constant_initializer(0.0))
            ...
            with tf.compat.v1.variable_scope('layer_2'):
                weights = tf.compat.v1.get_variable("w_2", [3, 3, 32, 64],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights, update_collection=update_collection)
                bias = tf.compat.v1.get_variable("b_2", [64], initializer=tf.compat.v1.constant_initializer(0.0))
                conv2_vi = tf.nn.conv2d(conv1_vi, weights, strides=[1, 2, 2, 1], padding='VALID') + bias
                conv2_vi = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv2_vi)
                conv2_vi = lrelu(conv2_vi)
                # print(conv2_vi.shape)
            with tf.compat.v1.variable_scope('layer_3'):
                weights = tf.compat.v1.get_variable("w_3", [3, 3, 64, 128],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights, update_collection=update_collection)
                bias = tf.compat.v1.get_variable("b_3", [128], initializer=tf.compat.v1.constant_initializer(0.0))
                conv3_vi = tf.nn.conv2d(conv2_vi, weights, strides=[1, 2, 2, 1], padding='VALID') + bias
                conv3_vi = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv3_vi)
                conv3_vi = lrelu(conv3_vi)
                # print(conv3_vi.shape)
            with tf.compat.v1.variable_scope('layer_4'):
                weights = tf.compat.v1.get_variable("w_4", [3, 3, 128, 256],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights, update_collection=update_collection)
                bias = tf.compat.v1.get_variable("b_4", [256], initializer=tf.compat.v1.constant_initializer(0.0))
                conv4_vi = tf.nn.conv2d(conv3_vi, weights, strides=[1, 2, 2, 1], padding='VALID') + bias
                conv4_vi = tf.keras.layers.BatchNormalization(epsilon=1e-5, momentum=0.9)(conv4_vi)
                conv4_vi = lrelu(conv4_vi)
                conv4_vi = tf.reshape(conv4_vi, [self.batch_size, 6 * 6 * 256])
            with tf.compat.v1.variable_scope('line_5'):
                weights = tf.compat.v1.get_variable("w_5", [6 * 6 * 256, 1],
                                                    initializer=tf.compat.v1.truncated_normal_initializer(stddev=1e-3))
                weights = weights_spectral_norm(weights, update_collection=update_collection)
                bias = tf.compat.v1.get_variable("b_5", [1], initializer=tf.compat.v1.constant_initializer(0.0))
